...
namespace SHHH.Cryptography.Cryptographers
{
    ...
    /// <summary>
    /// A Rihndael Cryptographer, Passphrase can be set in the app setting key "RijndaelSimple::PassPhrase", otherwise
    /// a default is used.
    /// And a 16 byte InitVector can be set in the app setting key "RijndaelSimple::16ByteInitVector", otherwise a
    /// default init vector is used.
    /// </summary>
    public class RijndaelCryptographer : ICryptographer
    {

        public RijndaelCryptographer(string passPhrase, string initVecor16Bytes)
        {
            if (string.IsNullOrWhiteSpace(passPhrase))
                throw new ArgumentException(Exceptions.MissingEmptyPassPhrase, "passPhrase");
            if (string.IsNullOrWhiteSpace(initVecor16Bytes) || initVecor16Bytes.Length != 16)
                throw new ArgumentException(Exceptions.InvalidInitVector, "initVector16Bytes");

            InitVector = initVecor16Bytes;
            PassPhrase = passPhrase;
        }
        /// <summary>
        /// Gets the pass phrase used to envrypt the data. A passphrase can be set in the AppSetting key "RijndaelSimple::PassPhrase".
        /// </summary>
        /// <value>The pass phrase.</value>
        private string PassPhrase { get; set; }

        /// <summary>
        /// Gets the init vector, a 16 byte string from the AppSetting key "RijndaelSimple::16ByteInitVector".
        /// </summary>
        /// <value>The init vector.</value>
        private string InitVector { get; set; }

        #region ICryptographer Members

        /// <summary>
        /// Encrypts the to encrypt string with the salt specfified.
        /// </summary>
        ...
        public string Encrypt(string salt, string toEncrypt)
        {
            ...
            return Encrypt(toEncrypt, PassPhrase, salt, "SHA1", 2, InitVector, 256);
        }
        ...
        public string Decrypt(string salt, string toDecrypt)
        {
            ...
            return Decrypt(toDecrypt, PassPhrase, salt, "SHA1", 2, InitVector, 256);
        }

        #endregion

        #region SampleCode
        public static string Encrypt(string toEncrypt, string passPhrase, string saltValue, string hashAlgorithm,
